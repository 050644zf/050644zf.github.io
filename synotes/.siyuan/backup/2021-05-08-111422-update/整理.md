...
{: id="20210507144138-hiyevx6" updated="20210507222708"}
...
| 算法                                                                                                                   | 主要思想                               |
| ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------- |
| [Sparsenet](http://www.cns.nyu.edu/~tony/vns/readings/olshausen-field-1996.pdf)                                        | 最大似然估计                           |
| [MOD](https://ieeexplore.ieee.org/abstract/document/760624/)                                                           | $l_o$范数衡量稀疏性<br />最小均方误差    |
| [K-SVD](https://freddy.cs.technion.ac.il/wp-content/uploads/2017/12/K-SVD-An-Algorithm-for-Designing-Overcomplete.pdf) | 正交匹配追踪(OMP)<br />奇异值分解(SVD) |
| [ODL](https://www.jmlr.org/papers/volume11/mairal10a/mairal10a.pdf)                                                    | 每次迭代仅处理一个样本                 |
...
MP 算法与OMP 算法具有相同的最优原子选择策略, 每次迭代均以测量矩阵$\phi$中与当次冗余向量$r_{t-1}$ 具有最大内积的列向量作为待估信号的支撑候选. 但是两者在残差信号的更新方式上不同, MP算法由其最新误差$r_t$ 与当次所选原子$\phi \lambda_t$的正交性($\left \langle r_t,φλ_t \right \rangle = \left \langle r_{t-1}-a_t,  \phi \lambda_t \right \rangle\ge 0$) 来保证算法的收敛性, 然而这点仅保证了冗余向量$r_t$与$\phi \lambda_ t$的正交性,而不能保证$r_t$ 与已选原子集合${\phi \lambda_1,\phi \lambda_2,\ldots,\phi \lambda_n}$的正交性, 这使得MP 求得的近似解在最优K项近似的意义下都是次最优的, 这也是MP 算法需要较多次数的迭代才能保证收敛的原因. OMP 算法通过递归的对已选原子集合正交化(见算法2中的第4步) 求出正交投影$P_t$, 利用$r_t = y_¡ P_ty$ 的残差更
新方式,克服了MP 算法的次最优性.
{: id="20210507221636-t90ve3u" updated="20210508103806"}
...
{: id="20210507215854-7711yf8" updated="20210507221636"}
...
ROMP 和SP 具有相同的原子选择机制, 每次迭代都以$\phi^Ty_r$(Tyr表示当前冗余) 中前 m 个绝对值最大的非零坐标所对应的原子作为候选; 其中,ROMP 利用正则化的方法找出候选原子集合中能量最大的原子集合$J_0%$, 在所有支撑集为$J_0$ 的稀疏信号中利用最小二乘法找出使得$||y-\phi x||_{l_2}$最小的解x作为当前估计, 并以$y -\phi x$作为残差更新公式, 没有”回溯” 的思想, 每次迭代都在一个“可靠” 候选集中选择出满足正则条件的原子添加到上次迭代中, 并且这些原子一直保持到迭代停止. SP与ROMP 在残差更新方式上则不同, 它以相邻两次迭代冗余的差作为迭代停止条件, 每次迭代中在当次迭代支撑和前次迭代支撑集合的并集( $T’$)上利用最小二乘求出当前估计$x=  y$, 可见SP采用了“回溯”思想, 与ROMP相比更好地保证了重建的最优性.
{: id="20210507203753-jvhsi4p" updated="20210508103613"}
...
{: id="20210507173604-f78mn2z" updated="20210507221426"}
...
StOMP 与CoSaMP 在本质上都采用了“代理”的思想, 即如果测量矩阵满足限制等容常数$\delta_m << 1$, 则向量$v = \phi^T\phi x = φ^Ty$ 可以看做待估信号x 的一个“代理”, 其合理性在于: 根据RIC 的定义, 当$\delta_m << 1$ 时, v 中每m 个元素构成的集合所包含的信息近似于x 中m 个元素所构成的集合,那么从v 中前m 个最大元所对应的支撑也应该对应于这些元素在x 中的支撑. StOMP 与CoSaMP的区别在于最优匹配原子的选择机制不同, StOMP通过设立软阈值$h = t_s\delta_s$ 直接判断入选的原子集合, 利用最小二乘求出待估信号在所得支撑上的系数, 其缺点在于阈值参数选择较难; CoSaMP 则在每次迭代中计算代理信号(即$φ^Tr_{s-1}$) 中前2m个最大元所对应的支撑与前次逼近解的支撑并作为当次候选支撑, 再利用最小二乘求出该支撑上的系数估计. 从算法6 的第4) 步和算法7 的第3) 步可以看出两者都采用了“回溯”思想.
{: id="20210507221426-lrqrudb" updated="20210508103833"}
...
稀疏表征模型在将 $l_0$ 范数替换为 $l_1$ 范数后约束优化问题形式（LS问题）：
{: id="20210507161024-ciqykya" updated="20210508101737"}
...
### 稀疏贝叶斯算法 SBL
{: id="20210507154348-y7mwmmi" updated="20210508095451"}

根据信号稀疏先验特性，将非线性优化问题转化为带有稀疏约束的线性回归问题的稀疏贝叶斯算法(SBL)，因其具有更高的重构精度被越来越多地用于压缩感知重构中。
{: id="20210507214956-jy6dp2e" updated="20210508095450"}
...
{: id="20210508095501-7lamukn"}

### 近似消息传递算法 AMP
{: id="20210507215016-zvark6m" updated="20210508095904"}

AMP算法对IST算法作了改进，既具有迭代阈值算法的低复杂度，同时也具备基追踪算法较强的信号重构能力
{: id="20210508101433-u7cvw5g" updated="20210508101915"}

![image.png](assets/image-20210508101429-edg3yol.png)
{: id="20210508101343-d81f4z4" updated="20210508101429"}

> 关于AMP算法的介绍：[AMP Tutorial](https://krzakala.github.io/cargese.io/AMP_Tutorial_18.pdf)
> {: id="20210508101341-7tb0txi" updated="20210508104409"}
{: id="20210508100559-ujzcc1e" updated="20210508101341"}

> Donoho D L, Maleki A, Montanari A. [Message-passing algorithms for compressed sensing[J]. Proceedings of the National Academy of Sciences](https://www.pnas.org/content/pnas/106/45/18914.full.pdf), 2009, 106(45): 18914-18919.
> {: id="20210508100529-0anu4z5" updated="20210508100544"}
{: id="20210508095905-fs5e3iw" updated="20210508100529"}

其它AMP算法：
{: id="20210508103954-6qxxkmt" updated="20210508104009"}

> **EP2AMP:** Meng X, Wu S, Kuang L, et al. [Concise derivation of complex Bayesian approximate message passing via expectation propagation](https://arxiv.org/pdf/1509.08658)[J]. arXiv preprint arXiv:1509.08658, 2015.
> {: id="20210508104010-5ox9wc5" updated="20210508104130"}
>
> **AMP-MMV:** Ziniel J, Schniter P. [Efficient high-dimensional inference in the multiple measurement vector problem](https://arxiv.org/pdf/1111.5272)[J]. IEEE Transactions on Signal Processing, 2012, 61(2): 340-354.
> {: id="20210508104133-79v5jgq" updated="20210508104219"}
>
> **TAP-AMP:** Krzakala F, Mézard M, Sausset F, et al. [Probabilistic reconstruction in compressed sensing: algorithms, phase diagrams, and threshold achieving matrices](https://arxiv.org/pdf/1206.3953)[J]. Journal of Statistical Mechanics: Theory and Experiment, 2012, 2012(08): P08009.
> {: id="20210508104222-7hmaobt" updated="20210508104335"}
{: id="20210508104009-0e9hdfw" updated="20210508104010"}

{: id="20210508102120-fh1l3xc"}

### 残差卷积神经网络 ResCNN
{: id="20210508100539-s788f80" updated="20210508102230"}

> Kim C, Park D, Lee H N. [Compressive sensing spectroscopy using a residual convolutional neural network](https://www.mdpi.com/1424-8220/20/3/594/pdf)[J]. Sensors, 2020, 20(3): 594.
> {: id="20210508102235-24bk814" updated="20210508102257"}
{: id="20210508102133-tq9eq2o" updated="20210508102235"}

{: id="20210508102301-ucwasxq"}

### 生成对抗网络 GAN
{: id="20210508102258-nrxi55o" updated="20210508102548"}

> Wu Y, Rosca M, Lillicrap T. [Deep compressed sensing[C]//International Conference on Machine Learning](http://proceedings.mlr.press/v97/wu19d/wu19d.pdf). PMLR, 2019: 6850-6860.
> {: id="20210508102612-cv06ndz" updated="20210508102624"}
{: id="20210508102549-kw5zdqq" updated="20210508102612"}

{: id="20210508102626-hz1dqiv"}
